...
class Net(nn.Module):
    def __init__(self, embedding, args):
        ...
        self.pep_encoder1_1 = nn.Sequential(
            nn.Conv1d(self.embedding_dim, self.n_filters, kernel_size=1,padding=0),
            nn.Sigmoid())
        self.pep_encoder1_3 = nn.Sequential(
            nn.Conv1d(self.embedding_dim, self.n_filters, kernel_size=3,padding=1),
            nn.Sigmoid())
        self.pep_encoder1_5 = nn.Sequential(
            nn.Conv1d(self.embedding_dim, self.n_filters, kernel_size=5,padding=2),
            nn.Sigmoid())
        self.pep_encoder1_7 = nn.Sequential(
            nn.Conv1d(self.embedding_dim, self.n_filters, kernel_size=7,padding=3),
            nn.Sigmoid())
        self.pep_encoder1_9 = nn.Sequential(
            nn.Conv1d(self.embedding_dim, self.n_filters, kernel_size=9,padding=4),
            nn.Sigmoid())
        ...
        self.encoder2_1 = nn.Sequential(
            nn.Conv1d(self.n_filters,self.n_filters,kernel_size=1,padding=0),
            nn.Sigmoid(),
    ...
    def forward(self, pep, tcr):
        ...
        peptcr = torch.cat((l_pool_max_1, l_pool_max_3, l_pool_max_5, l_pool_max_7, l_pool_max_9), -1)
