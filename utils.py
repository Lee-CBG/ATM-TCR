...
def write_blackbox_output_batchiter(loader, model, wf, device='cpu', ifscore=True):
    ...
    for batch in loader:
        ...
        for i in range(len(pred)):
            ...
            if ifscore:
                wf.writerow([pep_seq, tcr_seq, int(y[i]), int(pred[i]), float(score[i])])
            else:
...
def get_performance(score, label):
    ...
    precision1 = precision_score(label, label_pred, pos_label=1, zero_division=0)
    precision0 = precision_score(label, label_pred, pos_label=0, zero_division=0)
    ...
    auc = roc_auc_score(np.array(label), np.array(score)) if len(np.unique(np.array(label)))!=1 else -1
